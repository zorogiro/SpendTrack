...
**categories**: id (pk), name, icon (nullable), color (hex), monthly_budget (real, nullable, base currency)

**expenses**: id (pk), amount (real, original currency), currency ('TND'|'EUR'), amount_base (real), category_id (fk), note (nullable), date (ISO date), created_at (ISO timestamp), recurring_id (fk, nullable)
...
When compacting, preserve: the data model, the projection formula, current phase/TODO state, and any schema or architecture decisions. Drop verbose tool output and exploratory back-and-forth.
